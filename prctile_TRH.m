function [y_interp ] = prctile_TRH( A, pctile )
    ...
    %   INPUT    A        a vector or matrix of values
    %            pctile   a scalar or vector with the desired percentile(s)
    %                     range 1-100, 
    %   OUTPUT   y_interp the value at the input percentile rank or, when
    %                     those values exceed the percentiles in the range
    %                     of input data, the minimum or maximum values of A
    ...
    % scale from percentiles to values to use as indexes for array A
    pctile_x = pctile * n; 
